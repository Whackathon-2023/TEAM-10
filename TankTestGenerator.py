...
def Generate_Configuration():
    ...
    delivery_scenario_configuration = [
        {
            "tankSerialNumber": "33063",
            "deliveryLitres": "10500"  
        },
        {
            "tankSerialNumber": "33435",
            "deliveryLitres": "15245"
        },
        {
            "tankSerialNumber": "33261",
            "deliveryLitres": "17350"
        }
    ]
    ...
    all_configuration_items["DELIVERY_USAGE_SCENARIO"] = delivery_usage_scenario_configuration
...
def Simulate_Delivery_Scenario():
    ...
    for one_tank_scenario in delivery_scenario:
        ...
        tank_readings, constant_reading_litres, last_reading_time = Simulate_Constant_Readings(selected_tank_information, current_time)
...
def Simulate_Delivery_Usage_Scenario():
    ...
    day_numbers = [ one_scenario['dayNumber'] for one_scenario in delivery_usage_scenario ]
    max_day_number = int(max(day_numbers)) + 1
    
    # Get minimum start time from scenarios
    start_times = [ datetime.strptime(one_scenario['startTime'], "%H:%M") for one_scenario in delivery_usage_scenario ]
    min_start_time = min(start_times)
    
    # Calculate the start time of generating readings
    current_date_time = datetime.now()
    start_date = current_date_time - timedelta(days = max_day_number)
    start_time = datetime(start_date.year, start_date.month, start_date.day, min_start_time.hour, min_start_time.minute)
    ...
    sorted_delivery_usage_scenario = sorted(delivery_usage_scenario, key=lambda x: x['dayNumber'])
    
    
    for one_tank_information in tank_information:
        tank_readings, constant_reading_litres, last_reading_date_time = Simulate_Constant_Readings(one_tank_information, 
                                                                                                    start_time)
        ...
        for one_scenario in sorted_delivery_usage_scenario:
            ...
            activity_start_date_time = datetime(activity_start_date.year, activity_start_date.month, activity_start_date.day, 
                                               activity_start_time.hour, activity_start_time.minute)
            
            
            # If there is gap since the last constant reading and next activity, then generate more constant readings
            number_of_constant_readings = math.floor((activity_start_date_time - last_reading_date_time).total_seconds()
            ...
            if activity == 'Delivery':
                ...
                for i in range(number_of_hours_needed):
                    ...
                    litres_filled = rate_value if litres_to_fill > rate_value else litres_to_fill
                ...
                activity_end_date_time = datetime(activity_start_date.year, activity_start_date.month, activity_start_date.day, 
                                                    activity_end_time.hour, activity_end_time.minute)
